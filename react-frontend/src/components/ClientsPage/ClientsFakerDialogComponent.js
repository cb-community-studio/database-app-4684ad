...
import clientsFakerFactory from "./clientsFakerFactory";
...
const ClientsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = clientsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("clients").create(elem));
...
export default connect(null, mapDispatch)(ClientsFakerDialogComponent);
