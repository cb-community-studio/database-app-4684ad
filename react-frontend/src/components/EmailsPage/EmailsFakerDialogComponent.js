...
import emailsFakerFactory from "./emailsFakerFactory";
...
const EmailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = emailsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("emails").create(elem));
...
export default connect(null, mapDispatch)(EmailsFakerDialogComponent);
