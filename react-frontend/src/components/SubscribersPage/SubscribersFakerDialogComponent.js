...
import subscribersFakerFactory from "./subscribersFakerFactory";
...
const SubscribersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = subscribersFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("subscribers").create(elem));
...
export default connect(null, mapDispatch)(SubscribersFakerDialogComponent);
